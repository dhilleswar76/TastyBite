...
import { orderAPI, reservationAPI, menuAPI, contactAPI, eventAPI, reviewAPI, whatsappAPI } from '../services/api';
...
function AdminDashboard() {
  ...
  const [suggestedPhotoChoices, setSuggestedPhotoChoices] = useState([]);
  ...
  const [whatsAppDeliveryStatus, setWhatsAppDeliveryStatus] = useState({}); // { [orderNumber]: 'idle' | 'sending' | 'sent' }

  // Automated WhatsApp Gateway Modal State (Option B)
  const [showWhatsAppGatewayModal, setShowWhatsAppGatewayModal] = useState(false);
  const [whatsAppConfig, setWhatsAppConfig] = useState({
    enabled: true,
    apiToken: '',
    phoneNumberId: '',
    businessPhone: '+91 88856 36899',
    status: 'active',
  });
  ...
  const [whatsAppBotData, setWhatsAppBotData] = useState({
  ...
  const fetchLiveWhatsAppBotStatus = async () => {
    try {
      const res = await whatsappAPI.getStatus();
      if (res.data) {
        setWhatsAppBotData(res.data);
      }
    } catch (e) {
      console.warn('Could not fetch live WhatsApp Bot status:', e.message);
    }
  };

  useEffect(() => {
    fetchLiveWhatsAppBotStatus();
    let timer = null;
    if (showWhatsAppGatewayModal) {
      timer = setInterval(fetchLiveWhatsAppBotStatus, 2500);
    }
    return () => {
      if (timer) clearInterval(timer);
    };
  }, [showWhatsAppGatewayModal]);

  const handleOpenWhatsAppGatewayModal = async () => {
    setShowWhatsAppGatewayModal(true);
    fetchLiveWhatsAppBotStatus();
  };

  const handleLogoutWhatsAppBot = async () => {
    if (!window.confirm('Are you sure you want to unlink your WhatsApp device? You will need to re-scan the QR code.')) return;
    try {
      await fetch('/api/whatsapp/logout', { method: 'POST' });
      showNotification('Logged out from WhatsApp. Generating new QR Code...', 'info');
      setTimeout(fetchLiveWhatsAppBotStatus, 1500);
    } catch (err) {
      showNotification(err.message || 'Logout failed', 'error');
    }
  };

  const handleRestartWhatsAppBot = async () => {
    try {
      await fetch('/api/whatsapp/restart', { method: 'POST' });
      showNotification('Refreshing WhatsApp Pairing QR Code...', 'info');
      setTimeout(fetchLiveWhatsAppBotStatus, 1500);
    } catch (err) {
      showNotification(err.message || 'Restart failed', 'error');
    }
  };

  const handleSendTestWhatsAppBill = async () => {
    const cleanDigits = (testPhoneInput || '').replace(/[^0-9]/g, '');
    if (!cleanDigits || cleanDigits.length < 10) {
      showNotification('Please enter a valid 10-digit mobile number for test dispatch.', 'error');
      return;
    }
    setIsSendingTest(true);
    try {
      const res = await whatsappAPI.sendTest(cleanDigits);
      showNotification(`✅ Real Tax Invoice delivered directly to your WhatsApp (+91 ${cleanDigits.slice(-10)})! 📲`, 'success');
    } catch (err) {
      showNotification(err.message || 'Test dispatch failed. Please check WhatsApp QR connection.', 'error');
    } finally {
  ...
  // Smart Auto-Fill & Photo Match (Option 1): Generates calories, macros, description & 4K photo matches
  const handleAutoGenerateDishProfile = async () => {
    if (!menuFormData.name || !menuFormData.name.trim()) {
      showNotification('Please enter a Dish Name first to auto-fill details.', 'error');
      return;
    }
    ...
    try {
      ...
      setSuggestedPhotoChoices(profile.photoChoices || []);
      setImageUploadStatus(`✨ Auto-matched ${profile.photoChoices?.length || 4} 4K photos & ${profile.calories} kcal`);
      showNotification(`✨ Auto-matched 4K photos & calculated ${profile.calories} kcal for "${profile.name}"! ⚡`, 'success');
    } catch (err) {
      showNotification(err.message || 'Auto-fill failed. Please try again.', 'error');
    } finally {
  ...
  const handlePosSubmitOrder = async () => {
    ...
    const cleanPhone = (posCustomerPhone || '').replace(/[^0-9]/g, '');
    if (!cleanPhone || cleanPhone.length < 10) {
      showNotification('⚠️ Customer Mobile Phone Number is MANDATORY (10 digits) for billing & direct WhatsApp invoice.', 'error');
      return;
    }
  ...
  const handleShareWhatsAppReceipt = async (order, customPhone) => {
    if (!order) return;
    const orderKey = order.orderNumber || order._id || order.id || 'current';
    const phone = customPhone || order.customer?.phone || posCustomerPhone;
    const cleanPhone = phone ? String(phone).replace(/[^0-9]/g, '') : '';
    if (!cleanPhone || cleanPhone.length < 10) {
      showNotification('⚠️ Customer Mobile Phone Number is MANDATORY (10 digits) to send WhatsApp bill.', 'error');
      return;
    }

    setWhatsAppDeliveryStatus((prev) => ({ ...prev, [orderKey]: 'sending' }));
...
*Type:* ${order.customer?.orderType === 'dine-in' ? `Dine-In (Table #${order.customer?.tableNumber || '1'})` : 'Takeaway Parcel'}
...
Thank you for dining with TastyBite! ✨`;
    ...
    try {
      ...
      if (res && res.success) {
        setWhatsAppDeliveryStatus((prev) => ({ ...prev, [orderKey]: 'sent' }));
        showNotification(`✅ Bill delivered directly to WhatsApp (+91 ${cleanPhone.slice(-10)})! 📲`, 'success');
      } else if (res && res.needsLinking) {
        setWhatsAppDeliveryStatus((prev) => ({ ...prev, [orderKey]: 'error' }));
        showNotification('📲 Please scan the QR Code on screen to link your WhatsApp first.', 'info');
        handleOpenWhatsAppGatewayModal();
      } else {
        setWhatsAppDeliveryStatus((prev) => ({ ...prev, [orderKey]: 'error' }));
        showNotification(res?.error || 'Failed to dispatch bill. Please link your WhatsApp device.', 'error');
        handleOpenWhatsAppGatewayModal();
      }
    } catch (err) {
      setWhatsAppDeliveryStatus((prev) => ({ ...prev, [orderKey]: 'error' }));
      showNotification(err.message || 'Please link your WhatsApp device using the QR Code.', 'error');
      handleOpenWhatsAppGatewayModal();
    }
  };
  ...
  return (
    <div className="admin-page-container">
      ...
      <div className="admin-tabs-bar">
        ...
        <button
          type="button"
          className="admin-tab-btn whatsapp-gateway-trigger-btn"
          onClick={handleOpenWhatsAppGatewayModal}
          title="Configure automated silent WhatsApp billing & test on your phone"
        >
          📲 WhatsApp Gateway (Silent Mode)
        </button>
      </div>
                  ...
                  <div className="pos-bill-card">
                    ...
                    <div className="pos-guest-row">
                      ...
                      <input
                        ...
                        required
                        maxLength={10}
                        placeholder="* Mobile Phone (Mandatory 10-Digits)"
                        value={posCustomerPhone}
                        onChange={(e) => setPosCustomerPhone(e.target.value.replace(/[^0-9]/g, '').slice(0, 10))}
                        className={`pos-guest-input phone ${!posCustomerPhone || posCustomerPhone.length < 10 ? 'phone-missing' : 'phone-ok'}`}
                      />
                    </div>
      ...
      {showMenuModal && (
        <div className="admin-modal-overlay" onClick={() => setShowMenuModal(false)}>
          <div className="admin-modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Smart Culinary Auto-Fill Bar */}
            <div className="admin-ai-generator-bar">
              <div className="ai-bar-left">
                <span className="ai-badge-chip">⚡ Smart Auto-Fill &amp; 4K Photo Match</span>
                <p>Type the dish name below to instantly calculate energy calories (kcal), macros, spice level, description &amp; match authentic 4K food photos.</p>
              </div>

              <button
                type="button"
                className="btn-ai-auto-generate"
                onClick={handleAutoGenerateDishProfile}
                disabled={!menuFormData.name || !menuFormData.name.trim() || isGeneratingAI}
                title={!menuFormData.name?.trim() ? 'Please enter dish name below to enable auto-fill' : 'Auto-fill calories, macros and match photos'}
              >
                {isGeneratingAI ? '⚡ Matching Photos...' : '⚡ Auto-Fill & Match Photos'}
              </button>
            </div>
            ...
            <form onSubmit={handleSaveMenuItem} className="admin-modal-form">
              ...
              <div className="admin-dish-image-studio">
                <label className="studio-label">Dish Photo &amp; Presentation *</label>
                <div className="dish-image-studio-box">
                  <div className="dish-image-preview-holder">
                    ...
                    <span className="dish-image-type-tag">
                      {menuFormData.image?.startsWith('data:') ? '📸 Device Upload' : '✨ 4K Photo Asset'}
                    </span>
                  </div>
                ...
                {/* 1-Click Photo Choices Selector */}
                {suggestedPhotoChoices.length > 0 && (
                  <div className="suggested-photos-section">
                    <label className="suggested-photos-label">
                      📸 Choose Best Dish Photo ({suggestedPhotoChoices.length} Matches Found):
                    </label>
                    <div className="photo-choices-grid">
                      {suggestedPhotoChoices.map((choice, idx) => {
                        const isSelected = menuFormData.image === choice.file;
                        return (
                          <div
                            key={choice.id || idx}
                            className={`photo-choice-card ${isSelected ? 'active' : ''}`}
                            onClick={() => {
                              setMenuFormData({ ...menuFormData, image: choice.file });
                              setImageUploadStatus(`Selected: ${choice.name}`);
                            }}
                            title={`Select ${choice.name}`}
                          >
                            <img src={choice.file} alt={choice.name} className="photo-choice-thumb" />
                            <div className="photo-choice-info">
                              <span className="photo-choice-name">{choice.name}</span>
                              {isSelected && <span className="photo-badge-active">✔ Selected</span>}
                            </div>
                          </div>
                        );
                      })}
          ...
          <div className="receipt-modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="receipt-actions-bar no-print">
              ...
              <div className="receipt-header-btns">
                ...
                {(() => {
                  const billKey = generatedBillOrder?.orderNumber || generatedBillOrder?._id || 'current';
                  const deliveryState = whatsAppDeliveryStatus[billKey];
                  if (deliveryState === 'sent') {
                    return (
                      <button
                        type="button"
                        className="btn-whatsapp-bill sent"
                        onClick={() => handleShareWhatsAppReceipt(generatedBillOrder)}
                        style={{ background: '#10b981', color: '#fff' }}
                        title="Click to re-open customer WhatsApp chat"
                      >
      ...
      {/* --- AUTOMATED WHATSAPP GATEWAY HUB MODAL (OPTION 1: LINKED DEVICE) --- */}
      {showWhatsAppGatewayModal && (
        <div className="admin-modal-overlay" onClick={() => setShowWhatsAppGatewayModal(false)}>
          <div className="admin-modal-card whatsapp-gateway-modal" onClick={(e) => e.stopPropagation()}>
            <div className="admin-modal-header">
              <div className="modal-title-with-badge">
                <h3>📲 WhatsApp Linked Device Bot (Direct Silent Sending)</h3>
                <span className={`gateway-badge-active ${whatsAppBotData.isLinked ? 'connected' : 'waiting'}`}>
                  {whatsAppBotData.isLinked
                    ? `🟢 Linked: +${whatsAppBotData.connectedUser?.id?.split(':')[0] || 'Active'}`
                    : whatsAppBotData.connectionStatus === 'qr_ready'
                    ? '🟡 QR Code Ready (Scan to Link)'
                    : '⏳ Connecting...'}
                </span>
              </div>
              <button className="modal-close-x" onClick={() => setShowWhatsAppGatewayModal(false)}>&times;</button>
            </div>

            <div className="whatsapp-gateway-body">
              {/* Linked State Screen */}
              {whatsAppBotData.isLinked ? (
                <div className="gateway-connected-card">
                  <div className="connected-badge-icon">✅</div>
                  <h4>WhatsApp Device Connected &amp; Active!</h4>
                  <p>
                    Linked WhatsApp Account: <strong>+{whatsAppBotData.connectedUser?.id?.split(':')[0]}</strong>
                  </p>
                  <p className="connected-subtext">
                    Every bill generated or sent will now be delivered <strong>100% directly and silently in the background</strong> straight to the customer&apos;s phone number with zero redirects!
                  </p>

                  <div className="connected-actions-row">
                    <button
                      type="button"
                      className="btn-unlink-device"
                      onClick={handleLogoutWhatsAppBot}
                    >
                      🚪 Unlink / Switch WhatsApp Account
                    </button>
                  </div>
                </div>
              ) : (
                /* QR Code Scan Pairing Screen */
                <div className="gateway-qr-scan-card">
                  <h4>📱 Scan QR Code to Link Restaurant WhatsApp</h4>
                  <p>Follow these 3 easy steps on your phone to enable direct silent sending:</p>

                  <ol className="qr-steps-list">
                    <li>Open <strong>WhatsApp</strong> on your mobile phone.</li>
                    <li>Tap <strong>Settings ⚙️ (iPhone)</strong> or <strong>Three Dots ⋮ (Android)</strong> &gt; <strong>Linked Devices</strong>.</li>
                    <li>Tap <strong>Link a Device</strong> and point your camera at this QR Code below:</li>
                  </ol>

                  <div className="qr-code-display-holder">
                    {whatsAppBotData.qrCodeDataUrl ? (
                      <div className="qr-image-wrapper">
                        <img
                          src={whatsAppBotData.qrCodeDataUrl}
                          alt="Scan to link WhatsApp"
                          className="whatsapp-qr-img"
                        />
                        <span className="qr-scan-hint">📲 Scan with WhatsApp Camera</span>
                      </div>
                    ) : (
                      <div className="qr-loading-box">
                        <div className="loading-spinner"></div>
                        <p>Generating pairing QR Code...</p>
                      </div>
                    )}
                  </div>

                  <div className="qr-refresh-bar">
                    <button
                      type="button"
                      className="btn-refresh-qr"
                      onClick={handleRestartWhatsAppBot}
                    >
                      🔄 Refresh QR Code
                    </button>
                  </div>
              ...
              {/* Live Real-time Test Dispatcher */}
              <div className="gateway-section-card test-card">
                <h4>🧪 Test Real WhatsApp Delivery (Send to Your Mobile)</h4>
                <p>Enter your 10-digit mobile number below to test receiving a live Tax Invoice directly on your WhatsApp:</p>
                <div className="test-dispatch-row">
                  ...
                  <input
                    ...
                    maxLength="10"
                    onChange={(e) => setTestPhoneInput(e.target.value.replace(/[^0-9]/g, ''))}
                    className="test-phone-input"
                  />
                  <button
                    type="button"
                    className="btn-send-test-whatsapp"
                    onClick={handleSendTestWhatsAppBill}
                    disabled={isSendingTest || !testPhoneInput || testPhoneInput.length < 10 || !whatsAppBotData.isLinked}
                    title={!whatsAppBotData.isLinked ? 'Please scan QR code above first to link device' : 'Send real test message'}
                  >
                    {isSendingTest ? '⏳ Sending...' : '🚀 Send Test Bill to WhatsApp'}
                  </button>
                </div>
                {!whatsAppBotData.isLinked && (
                  <small className="qr-warning-note">⚠️ Please scan the QR code above with your WhatsApp first before testing.</small>
                )}
              </div>

              <div className="admin-modal-actions">
                <button type="button" className="btn-cancel" onClick={() => setShowWhatsAppGatewayModal(false)}>
                  Close
                </button>
              </div>
            </div>
